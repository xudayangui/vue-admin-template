<template>
	<section>
		...
		<!--新增、编辑-->
		<added-editor-dialog :dialogForm="dialogForm" :submitLoading="submitLoading" :dialogVisible.sync="addedEditorDialog" @getCancel="handleCancel" @submitHandle="submitAddedEditorForm"></added-editor-dialog>
	</section>
</template>
	...
	methods: {
		...
		*/
		async successList(params) {	
			...
			if (res.data.retcode === this.$config.RET_CODE.SUCCESS_CODE) {
				...
				this.$message({type: "error",message: res.data.retmsg});
			}
		},
		/*
		...
		*/
		editHandle(index, row) {
			...
			let params = row.id;
			return this.$message.error("请配置接口!");
			this.$api.systemModule.goodsManage.getDataById(params).then(res => {
				if (res.data.retcode === this.$config.RET_CODE.SUCCESS_CODE) {
					let data = res.data.data;
					//设置传给后台的数据
					this.dialogForm = {
		...
		*/
		deleteHandle(tipsText, params, cancelTips) {
			...
			this.$confirm(tipsText, "提示", {
				...
				return this.$message.error("请配置接口!");
				//请求删除账号接口
				this.$api.systemModule.goodsManage.batchEdit(params).then(res => {
					if (res.data.retcode === this.$config.RET_CODE.SUCCESS_CODE) {
				...
				this.$message({type: "info",message: cancelTips});
			});
		},
		/*
		...
		*/
		batchDeleteHandle() {
			if (this.checkedIds.length > 0) {
				...
				this.$message({type: "error",message: "请选择拍品"});
			}
		},
		/*
		...
		*/
		submitAddedEditorForm() {
			...
			if (this.handleType === "add") {
				//添加
				this.$api.systemModule.goodsManage.add(params).then(res => {
					if (res.data.retcode === this.$config.RET_CODE.SUCCESS_CODE) {
				...
				return this.$message.error("请配置接口!");
				//编辑
				this.$api.systemModule.goodsManage.edit(params).then(res => {
					if (res.data.retcode === this.$config.RET_CODE.SUCCESS_CODE) {
						this.submitLoading = false;
						this.$message({type: "success",message: res.data.retmsg});
						this.getList(); //再次请求列表
